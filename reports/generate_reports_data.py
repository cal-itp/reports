...
def get_dates_year_month(year:int, month: int) -> list:
  ...
  date_end = ((start_dt + timedelta(days=32)).replace(day=1) - timedelta(days=1)).strftime("%Y-%m-%d")
  # Publish date is first day of the next month.
  publish_date = (start_dt.replace(day=1) + timedelta(days=32)).replace(day=1).strftime('%Y-%m-%d')
...
def generate_feed_info(itp_id: int, publish_date):
  feed_info = (
    tbls.mart_gtfs_quality.idx_monthly_reports_site()
      ...
      >> select(_.organization_itp_id, _.organization_name, _.organization_website, _.publish_date, _.date_start, _.date_end, _.route_ct, _.stop_ct, _.no_service_days_ct, _.earliest_feed_end_date)
...
def generate_routes_changed(change_query):
  routes_changed = tbls.mart_gtfs_quality.fct_monthly_route_id_changes() >> change_query
  return routes_changed

def generate_stops_changed(change_query):
  routes_changed = tbls.mart_gtfs_quality.fct_monthly_stop_id_changes() >> change_query
  return routes_changed
...
def dump_report_data(itp_id: int, month: str, year: int, publish_date, date_start, date_end):
...
def generate_report_data_by_year_month(month, year, publish_date, date_start, end_date, index_report_file_path):
  month_index = get_month_index(index_report_file_path)
  for report in month_index['reports']:
    dump_report_data(int(month_index['itp_id']), month, year, publish_date, date_start, end_date)
...
if __name__ == "__main__":
  if file:
    ...
    if args.v:
      ...
      date_start, date_end, publish_date = dates[0], dates[1], dates[2]
      generate_report_data_by_year_month(month, year, publish_date, date_start, date_end)