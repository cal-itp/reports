...
#### Run the data
Execute `poetry run python generate_reports_data.py --year=2023 --month=02` to populate those output folders for a given month with the following files (per folder).
* 1_feed_info.json
...
All report data for every month can be generated by running `make data` (which just calls `poetry run python generate_reports_data.py`). This may be necessary when data or webpage changes necessitates re-generating prior months (for example, adding a new field).
